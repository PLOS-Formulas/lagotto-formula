...
{% set pmc_couch_url            = couch_base_url ~ '/pmc_usage_stats' %}
{% set twitter_couch_url        = couch_base_url ~ '/plos-tweetstream/_design/tweets/_view/by_doi?key="%{doi}"' %}
{% set relativemetric_couch_url = couch_base_url ~ '/relative_metrics/_design/relative_metric/_view/average_usage?key="%{doi}"' %}
{% set f1000_couch_url          = couch_base_url ~ '/f1000' %}

{% set mediacuration_host = salt['pillar.get']('environment:' ~ environment ~ ':mediacuration', '127.0.0.1') %}
{% set mediacuration_url  = 'http://' ~ mediacuration_host ~ '/api/v1?doi=%{doi}' %}

{% set camoflauge = '10.136.104.162' %}
...
citeulike_cfg['events_url']                     = 'http://{{ camoflauge }}/citeulike/doi/%{doi}'
...
scopus_cfg['url'] = 'https://{{ camoflauge }}/scopus/content/search/index:SCOPUS?query=DOI(%{doi})'
...
wos_cfg['url']                            = 'https://{{ camoflauge }}/wos/cps/xrpc'
...
wos_cfg['url_private']                    = 'https://{{ camoflauge }}/wos/cps/xrpc'
...
pmc_cfg['feed_url']   = 'https://{{ camoflauge }}/pmc/pmc/utils/publisher/pmcstat/pmcstat.cgi'
pmc_cfg['events_url'] = 'http://{{ camoflauge }}/pmc/pmc/articles/PMC%{pmcid}'
...
facebook_cfg['url']          = 'https://{{ camoflauge }}/facebook/v2.1/?access_token=%{access_token}&id=%{query_url}'
...
facebook_cfg['authentication_url'] = 'https://{{ camoflauge }}/facebook/oauth/access_token?client_id=%{client_id}&client_secret=%{client_secret}&grant_type=client_credentials'
...
wikipedia_cfg['url'] = 'http://{{ camoflauge }}/wikipedia/w/api.php?action=query&list=search&format=json&srsearch=%{query_string}&srnamespace=0&srwhat=text&srinfo=totalhits&srprop=timestamp&srlimit=1&lang=%{host}'
...
wikipedia_cfg['events_url'] = 'http://{{ camoflauge }}/wikipedia/w/index.php?search=%{query_string}&lang=en.wikipedia.org'
...
f1000_cfg['url'] = 'http://{{ camoflauge }}/f1000/linkout/PLOS-intermediate.xml'
...
f1000_cfg['feed_url'] = 'http://{{ camoflauge }}/f1000/PLOS-intermediate.xml'
f1000_cfg['queue']    = 'default'
f1000_cfg['url_db']   = '{{ f1000_couch_url }}'
f1000_cfg['url_feed'] = 'http://{{ camoflauge }}/f1000/linkout/PLOS-intermediate.xml'
f1000_cfg['tracked']  = '0'
